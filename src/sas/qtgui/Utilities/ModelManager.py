"""This module acts as a singleton object for managing the models in
    sasview.  As long as only the top level functions are called,
    there will only be a single manager and models added at one point
    in the code can be access anywhere throughout the project.
...
class Signaller(QObject):
    """All PyQt signals need to be attached to a class,
    so we need this Signaller object to allow our module
...
class ModelManagerBase:
    ...
    Do not actually instantiate this class.  This module will do that
    for you.  Instead, just use this module like a an object, calling
    the functions below this line.  This will create only a single
    Manager and ensure that it remains in sync across the entire
    program.
    ...
    def update(self):
        ...
        if len(new_plugins) > 0:
            print("New Plugins: "+str(new_plugins))
...
## Below are the most important functions for managing the models

def findModels():
    return __modelmanager.findModels()


def _getModelList():
    return __modelmanager._getModelList()


def is_changed():
    return __modelmanager.is_changed()


def update():
    return __modelmanager.update()


def plugins_reset():
    return __modelmanager.plugins_reset()


def populate_menu(modelmenu, event_owner):
    return __modelmanager.populate_menu(modelmenu, event_owner)


def _on_model(evt):
    return __modelmanager._on_model(evt)


def _get_multifunc_models():
    return __modelmanager._get_multifunc_models()


def get_model_list():
    return __modelmanager.get_model_list()


def get_model_name_list():
    return __modelmanager.get_model_name_list()


def get_model_dictionary():
    return __modelmanager.get_model_dictionary()
